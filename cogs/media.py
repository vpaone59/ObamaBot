...
ball_phrases = [
    ...
    "Status: Balling.",
    ":rotating_light: Baller alert :rotating_light:",
]
...
class Media(commands.Cog):
    ...
    async def pixel_image(self, ctx, pixel_count=16):
        ...
        rgb_colors = []
        non_gif_files = [
            f for f in obama_pics if not f.endswith(".gif") or f == "pixel_obama.png"
        ]
        ...
        try:
            ...
            colors = colorgram.extract(os.path.join(obama_dir, file), pixel_count)
        ...
        await ctx.send(
            f"```{file}, in {pixel_count} pixels!```",
            file=discord.File("gifs/obama/pixel_obama.png"),
        )
